...
public class Landmarks {
	...
	int MAXLANDMARKS = 3000;
	double MAXERROR = 0.5; // if a landmark is within 20 cm of another landmark its the same landmark
	public int MINOBSERVATIONS = 15; // Number of times a landmark must be observed to be recognized as a landmark 84
	int LIFE = 40;
	double MAX_RANGE = 1;

	int MAXTRIALS = 1000; // RANSAC: max times to run algorithm
	int MAXSAMPLE = 10; // RANSAC: randomly select X points
	int MINLINEPOINTS = 30; // RANSAC: if less than 40 points left don't bother trying to find consensus
							// (stop algorithm)
	double RANSAC_TOLERANCE = 0.05; // RANSAC: if point is within x distance of line its part of line
	int RANSAC_CONSENSUS = 30; // RANSAC: at least 30 votes required to determine if a line
	...
	public class landmark {
		...
		public double rangeError; // distance from robot position to the wall we are using as a landmark (to
									// calculate error)
		public double bearingError; // bearing from robot position to the wall we are using as a landmark (to
									// calculate error)
	...
	public int RemoveBadLandmarks(double[] laserdata, double[] robotPosition) {
		...
		Xbounds[0] = Math.cos((1 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange
				+ robotPosition[0];
		Ybounds[0] = Math.sin((1 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange
				+ robotPosition[1];
		Xbounds[1] = Xbounds[0]
				+ Math.cos((180 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange;
		Ybounds[1] = Ybounds[0]
				+ Math.sin((180 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange;
		Xbounds[2] = Math.cos((359 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange
				+ robotPosition[0];
		Ybounds[2] = Math.sin((359 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange
				+ robotPosition[1];
		Xbounds[3] = Xbounds[2]
				+ Math.cos((180 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange;
		Ybounds[3] = Ybounds[2]
				+ Math.sin((180 * degreesPerScan * conv) + (robotPosition[2] * Math.PI / 180)) * maxrange;
		/*
		 * // the below code starts the box 1 meter in front of robot Xbounds[0]
		 * =Math.cos((0 * degreesPerScan * conv)+(robotPosition[2]*Math.PI/180)) *
		 * maxrange+robotPosition[0]; Xbounds[1] =Xbounds[0]+Math.cos((180 *
		 * degreesPerScan * conv)+(robotPosition[2]*Math.PI/180)) * maxrange; Xbounds[0]
		 * =Xbounds[0]+Math.cos((180 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)); //make box start 1 meter ahead of
		 * robot Ybounds[0] =Math.sin((0 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)) * maxrange+robotPosition[1]; Ybounds[1]
		 * =Ybounds[0]+Math.sin((180 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)) * maxrange; Ybounds[0]
		 * =Ybounds[0]+Math.sin((180 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)); //make box start 1 meter ahead of
		 * robot Xbounds[2] =Math.cos((360 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)) * maxrange+robotPosition[0]; Xbounds[3]
		 * =Xbounds[2]+Math.cos((180 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)) * maxrange; Xbounds[2]
		 * =Xbounds[2]+Math.cos((180 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)); //make box start 1 meter ahead of
		 * robot Ybounds[2] =Math.sin((360 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)) * maxrange+robotPosition[1]; Ybounds[3]
		 * =Ybounds[2]+Math.sin((180 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)) * maxrange; Ybounds[2]
		 * =Ybounds[2]+Math.sin((180 * degreesPerScan *
		 * conv)+(robotPosition[2]*Math.PI/180)); //make box start 1 meter ahead of
		 * robot
		 */
		// now check DB for landmarks that are within this box
		// decrease life of all landmarks in box. If the life reaches zero, remove
		// landmark
		double pntx, pnty;
		for (int k = 0; k < DBSize + 1; k++) {
			...
			for (i = 0; i < 4; i++) {
				if (((((Ybounds[i] <= pnty) && (pnty < Ybounds[j])) || ((Ybounds[j] <= pnty) && (pnty < Ybounds[i])))
						&& (pntx < (Xbounds[j] - Xbounds[i]) * (pnty - Ybounds[i]) / (Ybounds[j] - Ybounds[i])
								+ Xbounds[i]))) {
	...
	/*
	 * public landmark[] UpdateAndAddLandmarks(double[] laserdata, double[]
	 * robotPosition) { //have a large array to keep track of found landmarks
	 * landmark[] tempLandmarks = new landmark[400]; for(int i=0;
	 * i<tempLandmarks.length;i++) tempLandmarks[i]= new landmark(); int totalFound
	 * = 0; double val = laserdata[0]; for (int i = 1; i < laserdata.length - 1;
	 * i++) { // Check for error measurement in laser data if (laserdata[i-1] < 8.1)
	 * if (laserdata[i+1] < 8.1) if ((laserdata[i-1] - laserdata[i]) +
	 * (laserdata[i+1] - laserdata[i]) > 0.5) tempLandmarks[i] =
	 * UpdateLandmark(laserdata[i], i, robotPosition); else if((laserdata[i-1] -
	 * laserdata[i]) > 0.3) tempLandmarks[i] = UpdateLandmark(laserdata[i], i,
	 * robotPosition); else if (laserdata[i+1] < 8.1) if((laserdata[i+1] -
	 * laserdata[i]) > 0.3) tempLandmarks[i] = UpdateLandmark(laserdata[i], i,
	 * robotPosition); } //get total found landmarks so you can return array of
	 * correct dimensions for(int i=0; i<tempLandmarks.length;i++)
	 * if(((int)tempLandmarks[i].id) !=-1) totalFound++; //now return found
	 * landmarks in an array of correct dimensions landmark[] foundLandmarks = new
	 * landmark[totalFound]; //copy landmarks into array of correct dimensions int j
	 * = 0; for(int i=0; i<((landmark[])tempLandmarks).length;i++)
	 * if(((landmark)tempLandmarks[i]).id !=-1) { foundLandmarks[j] =
	 * (landmark)tempLandmarks[i]; j++; } return foundLandmarks; }
	 */
	public landmark[] UpdateAndAddLandmarksUsingEKFResults(boolean[] matched, int[] id, double[] ranges,
	...
	private landmark UpdateLandmark(boolean matched, int id, double distance, double readingNo,
	...
	public landmark[] ExtractLineLandmarks(double[] laserdata, double[] robotPosition) {
		...
		/*
		 * //removes worst outliers (points which for sure aren't on any lines) for (int
		 * i = 2; i < laserdata.length - 1; i++) { // Check for error measurement in
		 * laser data if (laserdata[i] < 8.1)
		 * if(Math.Abs(laserdata[i]-lastreading)+Math.Abs(lastreading-lastlastreading) <
		 * 0.2) { linepoints[totalLinepoints] = i; totalLinepoints++; //tempLandmarks[i]
		 * = GetLandmark(laserdata[i], i, robotPosition); lastreading = laserdata[i];
		 * lastreading = laserdata[i-1]; } else { lastreading = laserdata[i];
		...
		while (noTrials < MAXTRIALS && totalLinepoints > MINLINEPOINTS) {
			...
			// one point randomly and then sample from neighbours within some defined radius
			int centerPoint = MAXSAMPLE + rnd.nextInt(totalLinepoints - 1 - MAXSAMPLE);
			rndSelectedPoints[0] = centerPoint;
			for (int i = 1; i < MAXSAMPLE; i++) {
				newpoint = false;
				while (!newpoint) {
					temp = centerPoint + (rnd.nextInt(2) - 1) * rnd.nextInt(MAXSAMPLE);
					for (int j = 0; j < i; j++) {
			...
			// y = a+ bx
			// LeastSquaresLineEstimate(laserdata, robotPosition, rndSelectedPoints,
			// MAXSAMPLE, ref a, ref b);
			LeastSquaresLineEstimate(laserdata, robotPosition, rndSelectedPoints, MAXSAMPLE, line);
			...
			if (totalConsensusPoints > RANSAC_CONSENSUS) {
				...
				LeastSquaresLineEstimate(laserdata, robotPosition, consensusPoints, totalConsensusPoints, line);
				// for now add points associated to line as landmarks to see results
				for (int i = 0; i < totalConsensusPoints; i++) {
					// tempLandmarks[consensusPoints[i]] =
					// GetLandmark(laserdata[consensusPoints[i]], consensusPoints[i],
					// robotPosition);
					// Remove points that have now been associated to this line
					// newLinePoints.CopyTo(linepoints, 0);
					System.arraycopy(newLinePoints, 0, linepoints, 0, newLinePoints.length);
					totalLinepoints = totalNewLinePoints;
				}
				// add line to found lines
				la[totalLines] = line.a;
				lb[totalLines] = line.b;
				totalLines++;
	...
	// private void LeastSquaresLineEstimate(double[] laserdata, double[]
	// robotPosition, int[] SelectedPoints,
	...
	private void LeastSquaresLineEstimate(double[] laserdata, double[] robotPosition, int[] SelectedPoints,
		...
		// DEBUG
		/*
		 * double[] testX = {0, 1}; double[] testY = {1, 1}; for(int i = 0; i < 2; i++)
		 * { //convert ranges and bearing to coordinates x = testX[i]; y = testY[i];
		 * sumY += y; sumYY += Math.Pow(y,2); sumX += x; sumXX += Math.Pow(x,2); sumYX
		 * += y*x; } a = (sumY*sumXX-sumX*sumYX)/(testX.length*sumXX-Math.Pow(sumX, 2));
		...
		line.b = (sumY * sumXX - sumX * sumYX) / (arraySize * sumXX - Math.pow(sumX, 2));
		line.a = (arraySize * sumYX - sumX * sumY) / (arraySize * sumXX - Math.pow(sumX, 2));
	}

	// private double DistanceToLine(double x, double y, double a, double b) {
	private double DistanceToLine(double x, double y, Line line) {
		/*
		 * //y = ax + b //0 = ax + b - y double d = Math.Abs((a*x - y +
		...
		// so aox + bo = ax + b => aox - ax = b - bo => x = (b - bo)/(ao - a), y = ao*(b
		// - bo)/(ao - a) + bo
			...
			if (laserdata[i - 1] < 8.1)
				if (laserdata[i + 1] < 8.1)
					if ((laserdata[i - 1] - laserdata[i]) + (laserdata[i + 1] - laserdata[i]) > 0.5)
						tempLandmarks[i] = GetLandmark(laserdata[i], i, robotPosition);
					else if ((laserdata[i - 1] - laserdata[i]) > 0.3)
						tempLandmarks[i] = GetLandmark(laserdata[i], i, robotPosition);
	...
	private landmark GetLineLandmark(double a, double b, double[] robotPosition) {
		...
		double bearingError = Math.atan((py - robotPosition[1]) / (px - robotPosition[0])) - robotPosition[2]; // do you
																												// subtract
																												// or
																												// add
																												// robot
	...
	private void GetClosestAssociation(landmark lm) { // given a landmark we find the closest landmark in DB
		...
		if (leastDistance == 99999)
			...
			lm.id = landmarkDB[closestLandmark].id;
			lm.totalTimesObserved = landmarkDB[closestLandmark].totalTimesObserved;
		}
	}

	private int GetAssociation(landmark lm) { // this method needs to be improved so we use innovation as a validation
												// gate
												// currently we just check if a landmark is within some predetermined
												// distance of a landmark in DB
		for (int i = 0; i < DBSize; i++) {
	...
	public int AlignLandmarkData(landmark[] extractedLandmarks, boolean[] matched, int[] id, double[] ranges,
			double[] bearings, double[][] lmrks, double[][] exlmrks) {
		...
		matched = new boolean[uniquelmrks];
